                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Select value={filterCategory} onValueChange={setFilterCategory}>
                            ...
                            <SelectContent>
                                ...
                                <SelectItem value="petunjuk_teknis">Petunjuk Teknis</SelectItem>
                                <SelectItem value="peraturan_univ">Peraturan Univ</SelectItem>
                            ...
                            <table className="w-full text-left text-sm border-collapse">
                                <thead>
                                    <tr className="border-b bg-slate-50 dark:bg-slate-900/50">
                                        ...
                                        <th className="px-6 py-3.5 font-semibold text-slate-700 dark:text-slate-300">Versi & Status</th>
                                    ...
                                    {filteredDocs.map((doc) => (
                                        ...
                                        >
                                            <td className="px-6 py-4 font-medium">
                                                <div>
                                                    <span className="block text-sm text-foreground hover:text-primary transition-colors">{doc.title}</span>
                                                    {doc.document_number && (
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    {doc.visibility === 'public' ? (
                                                        <Badge variant="outline" className="text-[10px] text-emerald-600 border-emerald-300 bg-emerald-50 dark:bg-emerald-950/30 flex items-center gap-1">
                                            ...
                                            <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                                                <div className="flex items-center justify-end gap-1.5">
                                                    {/* Review Button */}
                                                    <Button variant="outline" size="icon" className="h-8 w-8 text-primary hover:text-primary" onClick={() => openReview(doc)} title="Review Dokumen">
                                                        <Eye className="h-4 w-4" />
                                                    </Button>

                                                    {/* Download current version */}
                                                    {doc.versions.length > 0 && (
                                                        <Button variant="outline" size="icon" className="h-8 w-8" asChild title="Unduh Dokumen">
                                                    ...
                                                    <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => openHistory(doc)} title="Riwayat Versi">
                                                        <History className="h-4 w-4" />
                                                    </Button>

                                                    {!isStaff && (
                                                        <>
                                                            <Button variant="outline" size="icon" className="h-8 w-8 text-blue-600 hover:text-blue-700" onClick={() => openNewVersion(doc)} title="Unggah Versi Baru">
                                                                <ArrowUpCircle className="h-4 w-4" />
                                                            </Button>
                                                            <Button variant="outline" size="icon" className="h-8 w-8 text-red-600 hover:text-red-700" onClick={() => handleDelete(doc.id)} title="Hapus">
                            ...
                            <div className="h-12 px-4 flex items-center justify-between bg-slate-900 border-b border-slate-800 shrink-0 select-none">
                                <div className="flex items-center gap-3 min-w-0">
                                    <div className="flex items-center gap-2 truncate">
                                        <h3 className="text-sm font-semibold truncate text-slate-100">{selectedDoc.title}</h3>
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                    <Button 
                                        variant="ghost" 
                                        size="sm" 
                                        onClick={() => { setIsReviewOpen(false); openHistory(selectedDoc); }}
                                    ...
                                    {selectedDoc.versions && selectedDoc.versions.length > 0 && (
                                        <Button asChild size="sm" variant="secondary" className="h-8 text-xs bg-slate-800 text-slate-100 hover:bg-slate-700 border border-slate-700">
                                            <a href={route('documents.download', selectedDoc.versions[selectedDoc.versions.length - 1].id)}>
                            ...
                            <div className="flex-1 w-full h-full relative bg-slate-950 overflow-hidden">
                                {selectedDoc.versions && selectedDoc.versions.length > 0 ? (
                                    (() => {
                                        ...
                                        const isPdfOrImage = latestVer.file_path.endsWith('.pdf') || 
                                            latestVer.file_path.endsWith('.png') || 
                                            latestVer.file_path.endsWith('.jpg') || 
                                            latestVer.file_path.endsWith('.jpeg');

                                        if (isPdfOrImage) {
                                            return (
                                                <iframe 
                                                    src={route('documents.preview', latestVer.id)} 
                                            ...
                                            return (
                                                <div className="w-full h-full flex flex-col items-center justify-center p-6 bg-slate-900 text-slate-100 text-center">
                                                    ...
                                                    <p className="font-semibold text-lg">Format Dokumen Office (DOCX / XLSX)</p>
                                                    <p className="text-sm text-slate-300 max-w-md mt-2 mb-6 leading-relaxed">
                                                        Pratinjau langsung untuk format Office tidak tersedia di peramban. Silakan klik tombol di bawah untuk mengunduh dan membuka berkas.
                                                    </p>
                                                    <Button asChild size="default" variant="secondary" className="bg-primary text-primary-foreground hover:bg-primary/90">
                                                        <a href={route('documents.download', latestVer.id)}>
                                                            <Download className="h-4 w-4 mr-2" />
                                                            Unduh Berkas ({latestVer.version_number})
                                                        </a>
                                                    </Button>
                                                </div>
                                            );
                    ...
                    <form onSubmit={handleCreateSubmit} className="space-y-4">
                        {Object.keys(createForm.errors).length > 0 && (
                            <div className="p-3 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-md text-xs text-red-600 dark:text-red-400 space-y-1">
                                <span className="font-semibold block">Gagal mengunggah dokumen:</span>
                                {Object.entries(createForm.errors).map(([key, err], idx) => (
                                    <div key={idx}>• {err}</div>
                                ))}
                            </div>
                        )}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => createForm.setData('document_number', e.target.value)}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <Select
                                    ...
                                    onValueChange={(val: any) => createForm.setData('subcategory', val)}
                                >
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                                ...
                                <Select
                                    ...
                                    onValueChange={(val: any) => createForm.setData('visibility', val)}
                                >
                                    <SelectTrigger id="visibility">
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="public">Public (Ditampilkan)</SelectItem>
                        ...
                        <DialogFooter>
                            ...
                            <Button type="submit" disabled={createForm.processing}>
                                {createForm.processing ? 'Mengunggah...' : 'Unggah Dokumen'}
                            </Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
                    ...
                    {selectedDoc && (
                        <form onSubmit={handleNewVersionSubmit} className="space-y-4">
                            {Object.keys(versionForm.errors).length > 0 && (
                                <div className="p-3 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-md text-xs text-red-600 dark:text-red-400 space-y-1">
                                    <span className="font-semibold block">Gagal mengunggah versi baru:</span>
                                    {Object.entries(versionForm.errors).map(([key, err], idx) => (
                                        <div key={idx}>• {err}</div>
                                    ))}
                                </div>
                            )}
                            ...
                            <DialogFooter>
                                ...
                                <Button type="submit" disabled={versionForm.processing}>
                                    {versionForm.processing ? 'Mengunggah...' : 'Unggah Versi Baru'}
                                </Button>
                            </DialogFooter>
                        </form>
                    )}
